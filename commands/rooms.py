...
def create_room(db, user, password):
    with db:
...
def delete_room(db, user, room_id):
    with db:
...
def join_room(db, user, room_id, password):
    with db:
...
def remove_topic(db, user, room_id):
    with db:
        if not rooms_service.update_room(db.cursor(), user.user_id, room_id):
            print("Room doesn't exist or you are not the owner")


def rate_topic(db, user, room_id, rate):
    with db:
